...
__constant__ unsigned char d_yTable[64];
__constant__ unsigned char d_cbcrTable[64];

__global__ void forwardFDCKernel(const unsigned char* input, short* output, const unsigned char* quant_table, int width, int height) {
    ...
    if (x < width && y < height) {
        block[ty][tx] = (float)input[y * width + x] - 128.0f; // Center pixel values around zero
    } else {
        block[ty][tx] = 0.0f; // Fill out-of-bound values with zero
    }
    __syncthreads();

    // Perform DCT
    if (tx < 8 && ty < 8) {
        float alpha_u = (tx == 0) ? 1.0f / sqrtf(8.0f) : 0.5f;
        float alpha_v = (ty == 0) ? 1.0f / sqrtf(8.0f) : 0.5f;

        float temp = 0.0f;
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                temp += block[j][i] *
                        cosf((2.0f * i + 1.0f) * tx * M_PI / 16.0f) *
                        cosf((2.0f * j + 1.0f) * ty * M_PI / 16.0f);
            }
        }

        temp *= alpha_u * alpha_v;
        int zigzag_idx = d_ZigZag[ty * 8 + tx];
        int global_idx = (blockIdx.y * (width / 8) + blockIdx.x) * 64 + zigzag_idx;

        // Quantization
        output[global_idx] = (short)roundf(temp / quant_table[zigzag_idx]);
    }
}
...
bool JpegEncoder::encodeToJPG(const char* fileName, int quality_scale)
{
	...
	unsigned char *d_y, *d_cr, *d_cb;
	
	y = (unsigned char*) malloc(m_width * m_height * sizeof(unsigned char));
	cr = (unsigned char*) malloc(m_width * m_height * sizeof(unsigned char));
	cb = (unsigned char*) malloc(m_width * m_height * sizeof(unsigned char));

	cudaMalloc((void**)&d_y, m_width * m_height * sizeof(unsigned char));
	cudaMalloc((void**)&d_cr, m_width * m_height * sizeof(unsigned char));
	cudaMalloc((void**)&d_cb, m_width * m_height * sizeof(unsigned char));

	cudaMemcpyToSymbol(d_ZigZag, ZigZag, sizeof(ZigZag));
	cudaMemcpyToSymbol(d_yTable, m_YTable, sizeof(m_YTable));
    cudaMemcpyToSymbol(d_cbcrTable, m_CbCrTable, sizeof(m_CbCrTable));
	...
	for(int yPos=0; yPos<m_height; yPos+=8)
	{
		for (int xPos=0; xPos<m_width; xPos+=8)
		{
			...
			memcpy(&y[start], yData, 64);
			memcpy(&cr[start], crData, 64);
			memcpy(&cb[start], cbData, 64);
		}
	}
	

	cudaMemcpy(d_y, y, m_width * m_height * sizeof(unsigned char), cudaMemcpyHostToDevice);
	cudaMemcpy(d_cr, cr, m_width * m_height * sizeof(unsigned char), cudaMemcpyHostToDevice);
	cudaMemcpy(d_cb, cb, m_width * m_height * sizeof(unsigned char), cudaMemcpyHostToDevice);
	...
	dim3 blockSize(8, 8); // Each block handles an 8x8 block
	dim3 gridSize((m_width + 7) / 8, (m_height + 7) / 8); // One block per 8x8 image block

	forwardFDCKernel<<<gridSize, blockSize>>>(d_y, d_yQuant, d_yTable, m_width, m_height);
	forwardFDCKernel<<<gridSize, blockSize>>>(d_cr, d_crQuant, d_cbcrTable, m_width, m_height);
	forwardFDCKernel<<<gridSize, blockSize>>>(d_cb, d_cbQuant, d_cbcrTable, m_width, m_height);
	
	cudaDeviceSynchronize();

	short *yQuant, *cbQuant, *crQuant;
	yQuant = (short*) malloc(m_width * m_height * sizeof(short));
	crQuant = (short*) malloc(m_width * m_height * sizeof(short));
	cbQuant = (short*) malloc(m_width * m_height * sizeof(short));
	...
	for(int yPos=0; yPos<m_height; yPos+=8)
	{
		for (int xPos=0; xPos<m_width; xPos+=8)
		{
			...
			_doHuffmanEncoding(&yQuant[start], prev_DC_Y, m_Y_DC_Huffman_Table, m_Y_AC_Huffman_Table, outputBitString, bitStringCounts); 
			...
			_doHuffmanEncoding(&cbQuant[start], prev_DC_Cb, m_CbCr_DC_Huffman_Table, m_CbCr_AC_Huffman_Table, outputBitString, bitStringCounts);
			_write_bitstring_(outputBitString, bitStringCounts, newByte, newBytePos, fp);

			//Cr通道压缩(crData -> crQuant)
			// _foword_FDC(crData, crQuant, m_CbCrTable);
			_doHuffmanEncoding(&crQuant[start], prev_DC_Cr, m_CbCr_DC_Huffman_Table, m_CbCr_AC_Huffman_Table, outputBitString, bitStringCounts);
			_write_bitstring_(outputBitString, bitStringCounts, newByte, newBytePos, fp);
		}
	}
