...
__global__ void dctKernel(const unsigned char* image_data, short* output_data, const unsigned char* quant_table, int width, int height) {
    ...
    // Load the 8x8 block into shared memory
...
__global__ void rgbToYCbCrKernel(const unsigned char* rgbBuffer, unsigned char* y_channel, unsigned char* cb_channel, unsigned char* cr_channel, int width, int height) {
    ...
    if (x < width && y < height) {
		...
		y_channel[y * width + x] = static_cast<unsigned char>(fminf(fmaxf(roundf(Y), 0.0f), 255.0f));
		cb_channel[y * width + x] = static_cast<unsigned char>(fminf(fmaxf(roundf(Cb), 0.0f), 255.0f));
		cr_channel[y * width + x] = static_cast<unsigned char>(fminf(fmaxf(roundf(Cr), 0.0f), 255.0f));
...
bool JpegEncoder::encodeToJPG(const char* fileName, int quality_scale) {
    ...
    yQuantizedData = new short[m_width * m_height];
    cbQuantizedData = new short[m_width * m_height];
    crQuantizedData = new short[m_width * m_height];
    ...
    // Define grid and block dimensions for color transform
    dim3 blockDimColor(16, 16);
    dim3 gridDimColor(m_width / 16, m_height / 16);

    // Launch the kernel for RGB to YCbCr conversion
    ...
    // Free RGB device memory
    cudaFree(d_rgbBuffer);

    // Allocate memory on the device for DCT
    cudaMalloc((void**)&d_y_output, m_width * m_height * sizeof(short));
    cudaMalloc((void**)&d_cb_output, m_width * m_height * sizeof(short));
    cudaMalloc((void**)&d_cr_output, m_width * m_height * sizeof(short));
    cudaMalloc((void**)&d_quant_table_Y, 64 * sizeof(unsigned char));
    ...
    // Copy quantization tables to device
    ...
    delete[] yQuantizedData;
    delete[] cbQuantizedData;
    delete[] crQuantizedData;
